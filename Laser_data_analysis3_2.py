# Created 14/09/2021
# Fabio Federici
# I change that I fix the transmission factor of the window.
# I measured on 2021/09/14 to be between 6.4/6.8 and 6.3/6.8, so I don't need to fit for it.
...
minimum_ON_period_list = [2,0.1]	# seconds
...
for cases_to_include in all_cases_to_include:
	...
	def calculate_laser_power_given_parameters_1(trash,search_emissivity,search_thickness_over_diffusivity,search_defocused_to_focused_power):
		print([search_emissivity,search_thickness_over_diffusivity,search_defocused_to_focused_power])
		...
		for index in range(len(all_laser_to_analyse_power_end)):
			...
			if False:
				...
				all_fitted_power.append(np.array([peak-through,through+all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power])/(all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power))
	...
	y = np.concatenate([np.ones_like(all_laser_to_analyse_power_end),np.ones_like(all_laser_to_analyse_power_end)])
	...
	sigma = np.array([sigma,sigma*2]).T.flatten()
	bds = [[0.7,0.1*2.5e-6/Ptthermaldiffusivity,0.8],[1,10*2.5e-6/Ptthermaldiffusivity,1.2]]
	guess=[0.98,1.5*2.5e-6/Ptthermaldiffusivity,1]
	fit = curve_fit(calculate_laser_power_given_parameters_1, x, y, sigma=sigma, p0=guess,bounds=bds,maxfev=int(1e6),verbose=2,diff_step=[0.00001,guess[0]/10000,0.0001])
	...
	emissivity_first_stage, thickness_over_diffusivity_first_stage,defocused_to_focused_power_first_stage = fit[0]
	...
	def calculate_laser_power_given_parameters(trash,search_thickness):
		...
		for index in range(len(all_laser_to_analyse_power_end)):
			...
			if False:
				# temp = max(1,int((len(powernoback)-len(powernoback)//frames_for_one_pulse*frames_for_one_pulse)/2))
				footprint = np.ones((int(frames_for_one_pulse*experimental_laser_duty)))
				totalpower_filtered_2 = generic_filter(powernoback,np.mean,footprint=footprint)
				peaks_high = find_peaks(totalpower_filtered_2,distance=frames_for_one_pulse*0.95)[0]
				peaks_high = peaks_high[np.logical_and(peaks_high>frames_for_one_pulse/2,peaks_high<len(powernoback)-frames_for_one_pulse/2)].tolist()
				peaks_low = find_peaks(-totalpower_filtered_2,distance=frames_for_one_pulse*0.95)[0]
				peaks_low = peaks_low[np.logical_and(peaks_low>frames_for_one_pulse/2,peaks_low<len(powernoback)-frames_for_one_pulse/2)].tolist()
				all_fitted_power.append(np.array([totalpower_filtered_2[peaks_high].mean()-totalpower_filtered_2[peaks_low].mean(),totalpower_filtered_2[peaks_low].mean()+all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power])/(all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power))
			else:
				time_axis = (time_of_experiment[1:-1]-time_of_experiment[1])/1e6
				totalpower_filtered_2_full = generic_filter(powernoback,np.mean,size=[max(1,int(frames_for_one_pulse*experimental_laser_duty))])
				totalpower_filtered_2 = totalpower_filtered_2_full[int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5):-max(1,int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5))]
		...
		return np.concatenate(all_fitted_power)


	x = np.arange(len(all_laser_to_analyse_power_end)*2)
	y = np.concatenate([np.ones_like(all_laser_to_analyse_power_end)]*2)
	# weigth = np.array([np.ones_like(all_laser_to_analyse_power_end)*1,np.ones_like(all_laser_to_analyse_power_end)*0.1,np.ones_like(all_laser_to_analyse_power_end)*0.001*sharpness_degradation_high_frequency]).T.flatten()
	# sigma = np.ones_like(y)
	...
	if cases_to_include==['laser22','laser25','laser33','laser30']:
		diffusivity_second_stage = 1.1703748188219123e-05
		thickness_over_diffusivity_first_stage = 0.16943683874955529
		thickness_second_stage = thickness_over_diffusivity_first_stage * diffusivity_second_stage
		thickness_second_stage = 1.9830460945326834e-06
		emissivity_first_stage = 0.999999999
		defocused_to_focused_power_first_stage = 'gna'

	if cases_to_include==['laser34', 'laser35']:
		diffusivity_second_stage = 1.0335550297300824e-05
		thickness_over_diffusivity_first_stage = 0.16707771171990876
		diffusivity_second_stage = thickness_second_stage/thickness_over_diffusivity_first_stage
		thickness_second_stage = 1.7268400930390444e-06
		emissivity_first_stage = 0.9999999999999979
		defocused_to_focused_power_first_stage = 0.8000002329072671

	if cases_to_include==['laser22', 'laser25', 'laser33', 'laser30', 'laser34', 'laser35']:
		diffusivity_second_stage = 1.1556885537637909e-05
		thickness_over_diffusivity_first_stage = 0.1685594020100056
		diffusivity_second_stage = thickness_second_stage/thickness_over_diffusivity_first_stage
		thickness_second_stage = 1.948021715322328e-06
		emissivity_first_stage = 0.9999999999999999
		defocused_to_focused_power_first_stage = 0.9486153952225768

	if cases_to_include==['laser22', 'laser25', 'laser34']:
		diffusivity_second_stage = 9.252393474150652e-06
		thickness_over_diffusivity_first_stage = 0.22455103169365442
		diffusivity_second_stage = thickness_second_stage/thickness_over_diffusivity_first_stage
		thickness_second_stage = 2.0776345002561644e-06
		emissivity_first_stage = 0.9999999864110151
		defocused_to_focused_power_first_stage = 1
		# modified by hand
		diffusivity_second_stage = 10e-6
		thickness_second_stage = 2.1e-06
	...
	for index in np.arange(len(all_laser_to_analyse_power_end)):
		...
		diffusion_first = partial_diffusion * thickness_second_stage
		diffusion = partial_diffusion * thickness_second_stage
		diffusion_std = partial_diffusion_std * thickness_second_stage
		timevariation = (1/diffusivity_second_stage)*thickness_second_stage * partial_timevariation
		timevariation_first = (1/Ptthermaldiffusivity)*thickness_second_stage * partial_timevariation
		timevariation_std = (1/diffusivity_second_stage)*thickness_second_stage * partial_timevariation_std
		powernoback = diffusion + timevariation + BBrad
