...
all_cases_to_include = [['laserK02']]	# all cases in the same location
...
if False:	# these are only the plots for the paper
	...
	nuc_plate_emissivity_range = np.array([1])
	...
	for i_nuc_plate_emissivity,nuc_plate_emissivity in enumerate(nuc_plate_emissivity_range):
		...
		# output = []
		for i_emissivity,aggregated_emissivity in enumerate(aggregated_emissivity_range):
			output[i_nuc_plate_emissivity].append([])
			try:
				full_saved_file_dict[type_of_calibration]['emissivity='+str(aggregated_emissivity)] = full_saved_file_dict[type_of_calibration]['emissivity='+str(aggregated_emissivity)].all()
			except:
				pass
			...
			for i_reference_temperature,reference_temperature in enumerate(reference_temperature_range):
				...
				# plt.figure()
				# plt.plot(time_partial,BB)
				# plt.plot(time_partial,BB_small,'--')
				# plt.plot(time_partial,partial_diffusion)
				# plt.plot(time_partial,partial_diffusion_small,'--')

				sigmaSB=5.6704e-08 #[W/(m2 K4)]
				dx=foil_position_dict['foilhorizw']/foil_position_dict['foilhorizwpixel']

				BB_small = (partial_BBrad_small + 2*sigmaSB*((273.15+reference_temperature)**4-(273.15+reference_temperature+reference_temperature_correction)**4)*((15*dx)**2 *np.pi)) *emissivity
				BB = (partial_BBrad + 2*sigmaSB*((273.15+reference_temperature)**4-(273.15+reference_temperature+reference_temperature_correction)**4)*((30*dx)**2 *np.pi)) *emissivity

				# if False:	# just exploratory stuff

				framerate = 1/np.median(np.diff(time_partial))
				frames_for_one_pulse = framerate/experimental_laser_frequency
				from scipy.signal import find_peaks, peak_prominences as get_proms
				start_loc = find_peaks(np.abs(partial_timevariation_small),distance=int(frames_for_one_pulse*0.9/2))[0]
				temp = np.abs(partial_timevariation_small[start_loc[1]-int(frames_for_one_pulse*0.9/2/20):start_loc[1]])>np.std(np.diff(partial_timevariation_small))*2
				time_before_laser_off = start_loc[1]-int(frames_for_one_pulse*0.9/2/20)+temp.argmax()-45
				partial_diffusion_smooth = median_filter(partial_diffusion,size=[81])
				partial_diffusion_small_smooth = median_filter(partial_diffusion_small,size=[81])
				BB_smooth = median_filter(BB,size=[81])
				BB_small_smooth = median_filter(BB_small,size=[81])
				teqkeq = (BB_smooth - BB_small_smooth)[time_before_laser_off]/(partial_diffusion_small_smooth-partial_diffusion_smooth)[time_before_laser_off]
				Plaser_measured = (BB_smooth+partial_diffusion_smooth*teqkeq)[time_before_laser_off]
				if teqkeq<0 or Plaser_measured<0:	# these can't be
					output[i_nuc_plate_emissivity][i_emissivity].append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan])
					continue

				partial_timevariation_smooth = median_filter(partial_timevariation,size=[5])
				teqkeq_over_kappaeq = Plaser_measured/(partial_timevariation_smooth.max())

				absorbed_power = teqkeq_over_kappaeq*partial_timevariation+teqkeq*partial_diffusion+BB
				absorbed_power_small = teqkeq_over_kappaeq*partial_timevariation_small+teqkeq*partial_diffusion_small+BB_small

				# plt.figure()
				# plt.plot(time_partial,BB_smooth)
				# plt.plot(time_partial,BB_small_smooth,'--')
				# plt.plot(time_partial,partial_diffusion_smooth)
				# plt.plot(time_partial,partial_diffusion_small_smooth,'--')

				# plt.figure()
				# plt.plot(time_partial,absorbed_power)
				# plt.plot(time_partial,absorbed_power_small,'--')
				# plt.plot(time_partial,absorbed_power-absorbed_power_small)

				output[i_nuc_plate_emissivity][i_emissivity].append( [np.sum(np.abs(absorbed_power_small-absorbed_power))/Plaser_measured/len(time_partial) , np.min(absorbed_power)/Plaser_measured , np.min(absorbed_power_small)/Plaser_measured , (np.std(absorbed_power[absorbed_power>Plaser_measured/2])+np.std(absorbed_power[absorbed_power<Plaser_measured/2]))/2/Plaser_measured , (np.std(absorbed_power_small[absorbed_power_small>Plaser_measured/2])+np.std(absorbed_power_small[absorbed_power_small<Plaser_measured/2]))/2/Plaser_measured , teqkeq,teqkeq_over_kappaeq,Plaser_measured,emissivity,reference_temperature] )
	output=np.array(output)

	# I think I need both ways of enforcing reality through the penalty
	target_temperature = 22.5
	penalty_1 = np.mean(output[:,:,:,0],axis=0)
	select_1 = penalty_1 < 1.2*np.nanmin(penalty_1)
	select_1[np.abs(output[0,:,:,-1]-target_temperature)>4] = False
	penalty_2 = np.nanmean(output[:,:,:,1]/output[:,:,:,3],axis=0)
	select_2 = np.abs(penalty_2) <5
	select_2[np.abs(output[0,:,:,-1]-target_temperature)>4] = False
	select = np.logical_and(select_1,select_2)
	select[np.abs(output[0,:,:,-1]-target_temperature)>4] = False
	penalty = np.mean(output[:,:,:,0],axis=0)
	penalty -= np.nanmin(penalty[select])
	penalty /= np.nanmax(penalty[select])
	# penalty *=2
	penalty += 1

	print(case_ID)
	print('nuc_plate_emissivity')
	print(nuc_plate_emissivity_range[0])
	print('reference_temperature')
	print(np.sum(output[0,:,:,-1][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2))
	print('emissivity')
	print(np.sum(output[0,:,:,-2][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2))
	print('Plaser_measured')
	print(np.sum(output[0,:,:,-3][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2))
	print('teqkeq_over_kappaeq')
	print(np.sum(output[0,:,:,-4][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2))
	print('teqkeq')
	print(np.sum(output[0,:,:,-5][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2))
	print('kappaeq')
	print((np.sum(output[0,:,:,-5][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2)) / (np.sum(output[0,:,:,-4][select]*(1/penalty[select]**2))/np.sum(1/penalty[select]**2)))

	plt.figure()
	plt.imshow(np.nanmean(output[:,:,:,0],axis=0).T,'rainbow',vmax=1.2*np.nanmin(np.mean(output[:,:,:,0],axis=0).T))
	plt.colorbar()
	plt.scatter(np.array([np.arange(np.shape(output[0])[0])]*np.shape(output[0])[1]).T,np.array([np.arange(np.shape(output[0])[1])]*np.shape(output[0])[0]).T.T,c=select_1,s=4)
	plt.title(case_ID+ 'spatial error')

	plt.figure()
	plt.imshow(np.nanmean(output[:,:,:,1]/output[:,:,:,3],axis=0).T,'rainbow',vmax=1.2*np.nanmin(np.mean(output[:,:,:,0],axis=0).T))
	plt.colorbar()
	plt.title(case_ID+ 'relative negative peak large')
	plt.scatter(np.array([np.arange(np.shape(output[0])[0])]*np.shape(output[0])[1]).T,np.array([np.arange(np.shape(output[0])[1])]*np.shape(output[0])[0]).T.T,c=select_2,s=4)

	plt.figure()
	plt.imshow(np.nanmean(output[:,:,:,2]/output[:,:,:,4],axis=0).T,'rainbow',vmax=1.2*np.nanmin(np.mean(output[:,:,:,0],axis=0).T))
	plt.colorbar()
	plt.title(case_ID+ 'relative negative peak small')
	plt.scatter(np.array([np.arange(np.shape(output[0])[0])]*np.shape(output[0])[1]).T,np.array([np.arange(np.shape(output[0])[1])]*np.shape(output[0])[0]).T.T,c=select_2,s=4)


	item = -3
	plt.figure()
	plt.imshow((output[5,:,:,item]).T,'rainbow',vmax=1)
	plt.colorbar()
	plt.title('fix NUC emiss')

	plt.figure()
	plt.imshow((output[:,10,:,item]).T,'rainbow')
	plt.colorbar()
	plt.title('fix emiss')

	plt.figure()
	plt.imshow((output[:,:,5,item]).T,'rainbow')
	plt.colorbar()
	plt.title('fix T0')
