...
for grid_resolution in [2]:
	...
	for shrink_factor_x in [3]:
		...
		for shrink_factor_t in [7]:
			...
			x_optimal_input = np.zeros((len(grid_data_masked_crop)+2))
			x_optimal_input[-2:] = original_x_optimal[-2:]
			# original_x_optimal[:-2][original_x_optimal[:-2]<0] = 0

			voxels_centre = np.mean(grid_data_masked_crop,axis=1)
			recompose_voxel_emissivity_input = np.zeros((len(np.unique(voxels_centre[:,0])),len(np.unique(voxels_centre[:,1]))))*np.nan
			dr = np.median(np.diff(np.unique(voxels_centre[:,0])))
			dz = np.median(np.diff(np.unique(voxels_centre[:,1])))
			...
			for i_r,r in enumerate(np.unique(voxels_centre[:,0])):
				for i_z,z in enumerate(np.unique(voxels_centre[:,1])):
					...
					if dist.min()<dist_mean/2:
						...
						x_optimal_input[index] = original_x_optimal[original_index]
						recompose_voxel_emissivity_input[i_r,i_z] = x_optimal_input[index]
			recompose_voxel_emissivity_input *= 4*np.pi	# this exist because the sensitivity matrix is built with 1W/str/m^3/ x nm emitters while I use 1W as reference, so I need to multiply the results by 4pi
			...
			plt.imshow(np.flip(np.flip(np.flip(np.transpose(recompose_voxel_emissivity_input,(1,0)),axis=1),axis=1),axis=0),'rainbow',extent=[grid_data_masked_crop[:,:,0].min(),grid_data_masked_crop[:,:,0].max(),grid_data_masked_crop[:,:,1].min(),grid_data_masked_crop[:,:,1].max()],vmax=2.77e5,vmin=-1.99e5)
			plt.plot(_MASTU_CORE_GRID_POLYGON[:, 0], _MASTU_CORE_GRID_POLYGON[:, 1], 'k')
			...
			fitted_foil_power = (np.dot(sensitivities_binned_crop,x_optimal_input[:-2])+x_optimal_input[-2]*select_foil_region_with_plasma*homogeneous_scaling+x_optimal_input[-1]*(selected_ROI.flatten())*homogeneous_scaling).reshape(foil_power_residuals.shape)
			...
			# foil_power_residuals_simulated = np.random.normal(loc=0,scale=np.ones_like(fitted_foil_power)*1.647)
			foil_power_residuals_std_interp = interp1d([0,5,10,15,20,60,100,150,200,250],[0.9,1.1,1.4,1.647,2,6,8,9,11,19],fill_value="extrapolate",bounds_error=False)
			foil_power_residuals_simulated = np.random.normal(loc=np.zeros_like(fitted_foil_power),scale=foil_power_residuals_std_interp(fitted_foil_power))
			foil_power_residuals_simulated[sigma_powernoback.reshape(foil_power_residuals_simulated.shape)==1e10] = 0
			powernoback_full_orig = fitted_foil_power + foil_power_residuals_simulated
			sigma_powernoback_full = foil_power_std_interp(powernoback_full_orig)
			...
			plt.colorbar().set_label('input foil_power_residuals [W/m2]')
			plt.title(csv_file.name[-60:-28])
			plt.pause(0.01)
			...
			x_optimal_all,recompose_voxel_emissivity_all,y_opt_all,opt_info_all,voxels_centre = coleval.loop_fit_over_regularisation(prob_and_gradient,regolarisation_coeff_range,guess,grid_data_masked_crop,powernoback,sigma_powernoback,sigma_emissivity,pgtol=5e-8,factr=1e9)
