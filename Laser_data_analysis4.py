...
for cases_to_include in all_cases_to_include:
	...
	def calculate_laser_power_given_parameters(trash,search_thickness_1,search_thickness_2):
		...
		for index in range(len(all_laser_to_analyse_power_end)):
			...
			# totalpower_filtered_1 = totalpower_filtered_1[int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5):-max(1,int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5))]
			# plt.figure()
			# time_ON_after_SS = int(max(max(20,frames_for_one_pulse*experimental_laser_duty/8),frames_for_one_pulse*experimental_laser_duty - minimum_ON_period_list[0]*laser_framerate))
			# footprint = np.concatenate([np.ones((time_ON_after_SS)),np.zeros((time_ON_after_SS))])
			# totalpower_filtered_1 = generic_filter(powernoback,np.mean,footprint=footprint)
			# plt.plot(time_of_experiment[1:-1],generic_filter(timevariation,np.mean,size=[10]),'--',label='time')
			# plt.plot(time_of_experiment[1:-1],BBrad,label='BB')
			# plt.plot(time_of_experiment[1:-1],diffusion,label='diffusion')
			# # plt.plot(time_of_experiment[1:-1][int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5):-max(1,int(max(1,int(frames_for_one_pulse*experimental_laser_duty))*0.5))],totalpower_filtered_1)
			# plt.plot(time_of_experiment[1:-1],totalpower_filtered_1,label='totalpower_filtered_1')
			# plt.plot(time_of_experiment[1:-1],totalpower_filtered_2,label='totalpower_filtered_2')
			# plt.plot(time_of_experiment[1:-1][peaks_high+peaks_low],totalpower_filtered_2[peaks_high+peaks_low],'o')
			# plt.legend(loc='best', fontsize='small')
			# plt.pause(0.01)

			# all_fitted_power.append(np.array([totalpower_filtered_2.max()-totalpower_filtered_2.min(),totalpower_filtered_2.min()+all_laser_to_analyse_power_end[index]*power_reduction_window_first_stage])/(all_laser_to_analyse_power_end[index]*power_reduction_window_first_stage))
			all_fitted_power.append(np.array([totalpower_filtered_2[peaks_high].mean()-totalpower_filtered_2[peaks_low].mean(),totalpower_filtered_2[peaks_low].mean()+all_laser_to_analyse_power_end[index]*power_reduction_window_first_stage*defocused_to_focused_power_first_stage])/(all_laser_to_analyse_power_end[index]*power_reduction_window_first_stage*defocused_to_focused_power_first_stage))
	...
	for index in np.arange(len(all_laser_to_analyse_power_end)):
		...
		plt.plot([time_axis[0],time_axis[-1]],[peak+noise_amplitude]*2,'--k')
		plt.plot([time_axis[0],time_axis[-1]],[laser_to_analyse_power_end*defocused_to_focused_power*power_reduction_window_first_stage*defocused_to_focused_power]*2,'--r',label='power input')
		...
		plt.ylabel('power [W]')
		plt.grid()
		plt.title(laser_to_analyse+' in '+case_ID+'\nInput '+focus_status+', spot location '+ str([int(laser_location[0]),int(laser_location[2])]) +' power %.3gW, freq %.3gHz, duty%.3g\nHigh Power=%.3g+/-%.3gW, Low Power=%.3g+/-%.3gW, sharpness=%.3g' %(laser_to_analyse_power_end*defocused_to_focused_power*power_reduction_window_first_stage*defocused_to_focused_power,experimental_laser_frequency,experimental_laser_duty,fitted_power_2[2],fitted_power_2[3],fitted_power_2[0],fitted_power_2[1],sharpness_indicator))
		# plt.pause(0.01)
		figure_index+=1
