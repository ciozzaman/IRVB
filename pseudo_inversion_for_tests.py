		...
		for exp in np.linspace(-30,-6,num=73):
			...
			if not os.path.exists(path_sensitivity_original+'/svg_decomp_alpha_'+str(alpha)+'.npz'):
				...
				try:
					...
					try:
						print('Trying to recover SVG decomposition ' + path_sensitivity_original + '/svg_decomp_alpha_' + str(alpha))
						U, s, Vh = np.linalg.svd(test_matrix)
						np.savez_compressed(path_sensitivity_original + '/svg_decomp_alpha_' + str(alpha), U=U, s=s, Vh=Vh)
						print('stored SVG decomposition '+path_sensitivity_original+'/svg_decomp_alpha_'+str(alpha)+' recovered')
					except:
						print('the SVG decomposition of alpha = ' + str(alpha) + ' did not converge')
						continue
			...
			def linear1_wrap(fixed_point):
				import numpy as np
				def linear1(x, *params):

					# fixed_point = params[0]
					angular_coefficient = params[0]
					# c = params[2]
					# y0 = params[3]
					# n = params[4]
					return fixed_point - angular_coefficient * np.array(x)
				return linear1

			guess=[1E5]
			x_axis=np.linspace(1, len(s), len(s))
			test4, test3 = curve_fit(linear1_wrap(max(np.log(s))), x_axis, np.log(s), p0=guess,sigma=(x_axis)**0.5,maxfev=100000000)

			out1=linear1_wrap(max(np.log(s)))(x_axis,*test4)
			filter1=np.log(s)
			for index,value in enumerate(out1):
				if value<(1.5*np.log(s[index])):
					out1=out1[:index]
					filter1=filter1[index:]
					break
			...
			guess=[1E5]
			x_axis=np.linspace(1, len(filter1), len(filter1))
			test4, test3 = curve_fit(linear1_wrap(max(filter1)), x_axis, filter1, p0=guess,sigma=(x_axis)**1.4,maxfev=100000000)

			out2=linear1_wrap(max(filter1))(x_axis,*test4)
			filter2=filter1
			for index,value in enumerate(out2):
				if value<(1.1*(filter1[index])):
					out2=out2[:index]
			...
			for index, value in enumerate(s):
				if index % 30 == 0:
					s_shrink[0].append(index)
					s_shrink[1].append(value)
			s_shrink = np.array(s_shrink)
			plt.figure()
			plt.plot(s_shrink[0],s_shrink[1],'o')
			plt.plot(s)
			to_plot = s_shrink[1][:-2]-2*s_shrink[1][1:-1]+s_shrink[1][2:]
			plt.plot(s_shrink[0][1:-1],to_plot,'x')
			how_much_mobile_average = 30
			to_plot = np.convolve(to_plot, np.ones((how_much_mobile_average,)) / how_much_mobile_average, mode = 'valid')
			plt.plot(s_shrink[0][int(how_much_mobile_average/2):-1-int(how_much_mobile_average/2)],to_plot)
			plt.yscale('log')
			plt.pause(0.001)

			plt.figure()
			to_plot = np.log(to_plot)
			to_plot = to_plot[:-2]-2*to_plot[1:-1]+to_plot[2:]
			plt.plot(s_shrink[0][2+int(how_much_mobile_average/2):-1-int(how_much_mobile_average/2)],to_plot,'*')
			plt.pause(0.001)
			...
			for exp in np.linspace(-30, -6, num=73):
				...
				if os.path.exists(path_sensitivity_original + '/svg_decomp_alpha_' + str(alpha) + '.npz'):
					s = np.load(path_sensitivity_original + '/svg_decomp_alpha_' + str(alpha) + '.npz')['s']
					s_scaling = np.log(np.array(s)) - min(np.log(np.array(s)) - 1)
					s_scaling = s_scaling * 1 * (len(s) / max(s_scaling))
					s_scaling = s_scaling * np.linspace(int(min(s_scaling)), int(min(s_scaling)) + len(s), len(s))
					...
					for index in range(101, len(s_scaling) - 1, 1):
						...
						if (np.mean(s_scaling[index - 100:index - 10]) > s_scaling[index] and np.mean(
							...
							break
					treshold = s[find_minimum]
					treshold_all.append(treshold)
					alpha_all.append(alpha)
			plt.figure()
			plt.plot(alpha_all,treshold_all)
			start=coleval.find_nearest_index(alpha_all,1E-20)
			finish = coleval.find_nearest_index(alpha_all, 1E-14)
			test4, test3 = curve_fit(line, np.log(alpha_all[start:finish]), np.log(treshold_all[start:finish]), p0=[1,1],maxfev=100000000)
			plt.plot(alpha_all, np.exp(line(np.log(alpha_all),*test4)))
			plt.yscale('log')
			plt.xscale('log')
			plt.pause(0.0001)
