...
def movie_from_data(data,framerate,integration=1,xlabel=(),ylabel=(),barlabel=(),cmap='rainbow',timesteps='auto',extvmin='auto',extvmax='auto',mask=[0],time_offset=0,prelude='',vline=None,hline=None):
	import matplotlib.animation as animation
	import numpy as np
...
def ptw_to_dict(full_path):
	...
	for i in range(header.h_firstframe,header.h_lastframe):
		...
		data.append(np.flip(frame.flatten().reshape(np.shape(frame.T)).T,axis=0))
