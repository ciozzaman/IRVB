...
if False:	# manual collection of parameters
	...
	for name in cases_to_include:
		...
		all_path_reference_frames_ID.extend(collection_of_records[name]['reference_clear_ID'])
		...
		all_time_of_experiment.extend(collection_of_records[name]['time'])
		all_camera_frequency.extend(collection_of_records[name]['feqcamera'])
		all_camera_inttime.extend(collection_of_records[name]['inttimecamera'])
...
pathparams_BB='/home/ffederic/work/irvb/laser/Feb_2025/C-T_conversion/pixbypix_coeffs/Params_by_Kevin'	# original nuc measurements, calibration with wavewlength_top=5,wavelength_bottom=3
f = []
...
# reference_temperature_range = np.linspace(20,26,4)
reference_temperature_range = np.linspace(12,26,8)
# reference_temperature_range = np.linspace(12,18,4)
override = False
...
def function_a(index):
	...
	path_reference_frames_ID = all_path_reference_frames_ID[index]
	...
	camera_frequency = all_camera_frequency[index]
	camera_inttime = all_camera_inttime[index]
	...
	laser_to_analyse_power = power_interpolator(experimental_laser_voltage)	# W

	try:
		laser_dict = np.load(laser_to_analyse+'.npz')
		laser_dict.allow_pickle=True
		laser_counts_filtered = np.array([laser_dict['counts']])
		full_saved_file_dict = dict(laser_dict)
	except:
		print('the file '+laser_to_analyse+'.npz'+' for some reason is bad, read failed, regenerated')
		print('This needs to be done with the W7X virtual machine')
		exit()

	print(np.shape(laser_counts_filtered))
	...
	for type_of_calibration in ['BB_source_w_window']:
		...
		mean_time_of_experiment = time_of_experiment
		time_of_experiment = cp.deepcopy(full_saved_file_dict['time'])	# microseconds
		laser_digitizer = [0]*len(time_of_experiment)
		if np.diff(time_of_experiment).max()>np.median(np.diff(time_of_experiment))*1.1:	# adding frames if there are holes
			if len(np.unique(laser_digitizer))<2:
				print('the current mechanism to add miffing frames works only wit honly 1 digitizer')
				hole_pos = np.diff(time_of_experiment).argmax()
				while np.diff(time_of_experiment).max()>np.median(np.diff(time_of_experiment))*1.1:
					laser_counts_filtered = np.array([np.concatenate([laser_counts_filtered[0,:hole_pos+1], [laser_counts_filtered[0,hole_pos]+ (laser_counts_filtered[0,hole_pos+1]-laser_counts_filtered[0,hole_pos])*(np.median(np.diff(time_of_experiment))/np.diff(time_of_experiment[hole_pos:hole_pos+2]))], laser_counts_filtered[0,hole_pos+1:]])])
					time_of_experiment = np.concatenate([time_of_experiment[:hole_pos+1], [time_of_experiment[hole_pos]+np.median(np.diff(time_of_experiment))], time_of_experiment[hole_pos+1:]])
					hole_pos = np.diff(time_of_experiment).argmax()
				laser_digitizer = [0]*len(time_of_experiment)
			else:
				# this was only taking the data before or after the hole, not really adding it
				print('warning, if missing frames are detected this tool only cuts the data before or after the hole, rather than adding anything')
		...
		time_partial = (time_of_experiment-time_of_experiment[0])	# it's already in seconds
		laser_framerate = camera_frequency
		laser_int_time = camera_inttime

		print(np.shape(laser_counts_filtered))
		print(np.shape(time_of_experiment))
		...
		background_timestamps = [collection_of_records[path_reference_frames_ID]['time'][i_] for i_ in range(len(collection_of_records[path_reference_frames_ID]['time'])) if np.logical_and(np.abs(collection_of_records[path_reference_frames_ID]['feqcamera'][i_]-laser_framerate)<laser_framerate/100,np.abs(collection_of_records[path_reference_frames_ID]['inttimecamera'][i_]-laser_int_time)<laser_int_time/100)]
		background_timestamps = np.array(background_timestamps)
		background_temperatures = [collection_of_records[path_reference_frames_ID]['temperature'][i_] for i_ in range(len(collection_of_records[path_reference_frames_ID]['time'])) if np.logical_and(np.abs(collection_of_records[path_reference_frames_ID]['feqcamera'][i_]-laser_framerate)<laser_framerate/100,np.abs(collection_of_records[path_reference_frames_ID]['inttimecamera'][i_]-laser_int_time)<laser_int_time/100)]
		background_counts = []
		background_counts_std = []
		for i_ in range(len(collection_of_records[path_reference_frames_ID]['time'])):
			if  np.logical_and(np.abs(collection_of_records[path_reference_frames_ID]['feqcamera'][i_]-laser_framerate)<laser_framerate/100,np.abs(collection_of_records[path_reference_frames_ID]['inttimecamera'][i_]-laser_int_time)<laser_int_time/100):
				laser_dict = np.load(path_reference_frames[i_]+'.npz')
				laser_dict.allow_pickle=True
				data = dict(laser_dict)['counts']
				background_counts.append([np.mean(data,axis=0)])
				background_counts_std.append([np.std(data,axis=0)])

		background_counts = np.array(background_counts)
		background_counts_std = np.array(background_counts_std)
		relevant_background_files = [path_reference_frames[i_] for i_ in range(len(collection_of_records[path_reference_frames_ID]['time'])) if np.logical_and(np.abs(collection_of_records[path_reference_frames_ID]['feqcamera'][i_]-laser_framerate)<laser_framerate/100,np.abs(collection_of_records[path_reference_frames_ID]['inttimecamera'][i_]-laser_int_time)<laser_int_time/100)]
		...
		# 	flag_1 = [coleval.find_dead_pixels([data],treshold_for_bad_difference=100) for data in reference_background_full_1]
		# 	flag_2 = [coleval.find_dead_pixels([data],treshold_for_bad_difference=100) for data in reference_background_full_2]
		flag_1 = np.zeros_like(reference_background)	# no need to look at dead pixels, there aren't
		flag_2 = np.zeros_like(reference_background)
		...
		if False:	# I tried here to have an automatic recognition of the foil position and shape. I can't have a good enough contrast, so I abandon this route
			...
			foilhorizw=foil_position_dict['foilhorizw']	# m
			foilvertw=foil_position_dict['foilvertw']	# m
			...
			# plt.imshow(filters.sobel(testrot),'rainbow',origin='lower',vmax=5)
			...
			figure_index+=1
			plt.savefig(path_to_save_figures+laser_to_analyse[-31:-16] + path_to_save_figures2 + 'FIG'+str(figure_index)+'.eps')#, bbox_inches='tight')
			plt.close('all')
		...
		# for emissivity in emissivity_range[7:]:
			...
			if type_of_calibration == 'BB_source_w_window':
				temp = np.abs(parameters_available_int_time_BB-laser_int_time)<0.1
				...
				# Load parameters
				temp = pathparams_BB+'/'+temp+'/'
				params_dict=dict([])
				params_dict['coeff2'] = []
				params_dict['coeff2'].append(np.load(temp+'A1.npy'))
				params_dict['coeff2'].append(np.load(temp+'A2.npy'))
				params_dict['coeff2'] = np.array([np.transpose(params_dict['coeff2'] , axes=(1,2,0))])
				params_dict['errcoeff2'] = np.zeros((params_dict['coeff2'].shape[0],params_dict['coeff2'].shape[1],np.shape(params_dict['coeff2'])[2],np.shape(params_dict['coeff2'])[3],np.shape(params_dict['coeff2'])[3]))
				try:
			...
			if type_of_calibration == 'BB_source_w_window' or type_of_calibration == 'BB_source_w/o_window':
				reference_background_temperature,reference_background_temperature_std = coleval.count_to_temp_BB_multi_digitizer_no_reference(reference_background,reference_background_std,params_BB,errparams_BB,digitizer_ID,wavewlength_top=5,wavelength_bottom=3,inttime=int_time)
				if False:	# important: I need to fid first the emissivity to calculate the power!!
					...
					# reference_background_temperature,reference_background_temperature_std = coleval.count_to_temp_BB_multi_digitizer_no_reference(reference_background,reference_background_std,params_BB_with_emissivity,errparams_BB,digitizer_ID,wavewlength_top=5,wavelength_bottom=3,inttime=int_time)
					# np.mean(reference_background_temperature)
					ref_back_temp,ref_back_temp_std = coleval.count_to_temp_BB_multi_digitizer_no_reference(reference_background,reference_background_std,params_BB_with_emissivity,errparams_BB,digitizer_ID,wavewlength_top=5,wavelength_bottom=3,inttime=int_time)
			...
			# for reference_temperature in reference_temperature_range[:2]:
				...
				if len(laser_counts_filtered[0])>500:	# this is to avoid going out of memory
					...
					if type_of_calibration == 'BB_source_w_window' or type_of_calibration == 'BB_source_w/o_window':
						laser_temperature,laser_temperature_std = coleval.count_to_temp_BB_multi_digitizer(laser_counts_filtered,params_BB,errparams_BB,digitizer_ID,reference_background=reference_background,reference_background_std=reference_background_std,ref_temperature=ref_temperature,ref_temperature_std=ref_temperature_std,wavewlength_top=5,wavelength_bottom=3,inttime=int_time,counts_std = np.zeros_like(laser_counts_filtered))
					elif type_of_calibration == 'NUC_plate':
				...
				for i in range(len(digitizer_ID)):
					laser_counts_filtered_std.append(np.zeros_like(laser_counts_filtered[i]))
				...
				if False:
					laser_temperature_std_no_dead_pixels = cp.deepcopy(laser_temperature_std)
					laser_temperature_minus_background_no_dead_pixels = cp.deepcopy(laser_temperature_minus_background)
					laser_temperature_std_minus_background_no_dead_pixels = cp.deepcopy(laser_temperature_std_minus_background)
				else:
				...
				time_partial = (time_of_experiment_digitizer_ID-np.min(time_of_experiment_digitizer_ID))
				...
				if False:	# spatially resolved foil properties from Japanese producer
					...
					conductivityscaled=np.multiply(Ptthermalconductivity,np.ones((foilvertwpixel,foilhorizwpixel)))[1:-1,1:-1]
					reciprdiffusivityscaled=np.multiply(1/Ptthermaldiffusivity,np.ones((foilvertwpixel,foilhorizwpixel)))[1:-1,1:-1]
				elif True:	# homogeneous foil properties from foil experiments
					foilemissivityscaled=1*np.ones((foilvertwpixel,foilhorizwpixel))[1:-1,1:-1]
					foilthicknessscaled=(2.5/1000000)*np.ones((foilvertwpixel,foilhorizwpixel))[1:-1,1:-1]
					conductivityscaled=Ptthermalconductivity*np.ones((foilvertwpixel,foilhorizwpixel))[1:-1,1:-1]
					reciprdiffusivityscaled=(1/Ptthermaldiffusivity)*np.ones((foilvertwpixel,foilhorizwpixel))[1:-1,1:-1]
				...
				if False:	# I add this for the plots in the paper
					...
					plt.savefig(path_to_save_figures+laser_to_analyse[-31:-16] + path_to_save_figures2 + 'FIG_for_paper_1'+'.eps')#, bbox_inches='tight')
						...
						if i==0:
							...
							while attempt<20 and success==False:	# this while is just to retry when I get the memory error
								...
								try:
									...
									success = False
									print('failed calc_temp_to_power_BB_1 n'+str(attempt))
				...
				if False:	# other plots for the paper. i think i want to show that the area where the diffusion components cancels out grows in time
					...
					plt.axvline(x=20*dx*1000,color='y',linestyle='--')
					plt.title(preamble_4_prints+'Laser spot location in '+laser_to_analyse+'\n foil size '+str([foilhorizwpixel,foilvertwpixel])+'pixels, [%.3g,%.3g]mm\n laser located at [%.3g,%.3g]mm prelim radious %.3gmm\n' %(foilhorizwpixel*dx*1e3,foilvertwpixel*dx*1e3,horizontal_loc*dx*1e3,vertical_loc*dx*1e3,dr*dx*1e3))
					plt.savefig(path_to_save_figures+laser_to_analyse[-31:-16] + path_to_save_figures2 + 'FIG_for_paper_2'+'.eps')#, bbox_inches='tight')


				if focus_status == 'focused':
					dr_total_power_minimum = 15
					# dr_total_power = 17
				elif focus_status == 'partially_defocused':
				...
				BBtreshold = min(5,2*np.max(np.abs(np.array(test)[-5:])))
				...
				ax[0,0].axhline(y=laser_to_analyse_power*emissivity,color='k',linestyle='-',linewidth=2,label='Pin x emiss')
				...
				ax[0,0].set_ylabel('absorbed power [W]')
...
if False:
	...
	params_BB = np.ones((1,2,2,2))
	params_BB[:,:,:,0] = 5e-15
	params_BB[:,:,:,1] = 0
	errparams_BB = np.ones((1,2,2,4,4))
	digitizer_ID = np.array([0])
	reference_background = np.zeros((1,2,2))
	reference_background_std = np.ones((1,2,2))
	ref_temperature = 22
	ref_temperature_std = 1
	int_time = 0.5

	# My foil 17-09-2024
	x = np.array([193,154,117,73,46,191,191,190,189,192,190,183,183,190,183,183,193,194,145,146,146,146])
	y = np.array([156,157,158,159,161,126,95,68,23,159,159,160,165,164,165,165,185,221,222,235,198,242])
	z = np.array([1623,1662,1606,1592,1571,1617,1643,1714,1732,1635,1656,1570,1654,1604,1651,1612,1651,1678,1666,1611,1627,1548])#
	laser_counts_filtered = np.zeros((1,len(x),2,2))
	laser_counts_filtered[:,:,0,0] = z
	laser_temperature,laser_temperature_std = coleval.count_to_temp_BB_multi_digitizer(laser_counts_filtered,params_BB,errparams_BB,digitizer_ID,reference_background=reference_background,reference_background_std=reference_background_std,ref_temperature=ref_temperature,ref_temperature_std=ref_temperature_std,wavewlength_top=5,wavelength_bottom=3,inttime=int_time,counts_std = np.zeros_like(laser_counts_filtered))
	t = laser_temperature[0,:,0,0]-ref_temperature

	plt.figure()
	plt.scatter(x,y,c=t,s=50,marker='o',cmap='rainbow',)
	# plt.scatter(x,y,c=np.arange(len(t)),s=50,marker='o',cmap='rainbow')
	plt.plot([0,320,320,0,0],[0,0,255,255,0],'k')
	plt.colorbar()
	plt.gca().set_aspect('equal')
	plt.grid()

	print(str(np.mean(t)) + ' +/- ' + str(np.std(t)))
	select_close = [0,10, 11, 12, 13, 14, 15, 16]	# this selects the short range ones
	print(str(np.mean(t[[val in select_close for val in np.arange(len(t))]])) + ' +/- ' + str(np.std(t[[val in select_close for val in np.arange(len(t))]])))



	# Japanese foil 18-09-2024
	x = np.array([161,160,160,161,160,160,160,161,158,156,126,103,76,59,50,162,162,164,185,218,238,157,282,162,157,156,156,157,156])
	y = np.array([135,114,114,98,81,71,42,136,135,135,136,138,139,139,139,165,191,222,135,133,133,133,131,138,138,138,141,140,140])
	z = np.array([673,791,791,835,890,942,760,688,653,683,613,567,497,446,413,863,820,536,695,398,636,613,482,692,705,749,701,724,721])#
	laser_counts_filtered = np.zeros((1,len(x),2,2))
	laser_counts_filtered[:,:,0,0] = z
	laser_temperature,laser_temperature_std = coleval.count_to_temp_BB_multi_digitizer(laser_counts_filtered,params_BB,errparams_BB,digitizer_ID,reference_background=reference_background,reference_background_std=reference_background_std,ref_temperature=ref_temperature,ref_temperature_std=ref_temperature_std,wavewlength_top=5,wavelength_bottom=3,inttime=int_time,counts_std = np.zeros_like(laser_counts_filtered))
	t = laser_temperature[0,:,0,0]-ref_temperature
	...
	print(str(np.mean(t)) + ' +/- ' + str(np.std(t)))
	select_close = [0, 7, 8,9, 23,24,25,26,27,28]	# this selects the short range ones
	print(str(np.mean(t[[val in select_close for val in np.arange(len(t))]])) + ' +/- ' + str(np.std(t[[val in select_close for val in np.arange(len(t))]])))
