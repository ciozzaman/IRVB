...
all_j
				...
				if False:	# development plots
					...
					m3[m3>0]=0
					plt.figure()
					plt.imshow(rotate(np.sum(sensitivities_binned_crop*m3,axis=1).reshape(np.shape(powernoback_full[i_t])),-90),'rainbow',origin='lower')
					plt.plot([ROI[0,0]-0.5,ROI[0,1]-0.5,ROI[0,1]-0.5,ROI[0,0]-0.5,ROI[0,0]-0.5],[ROI[1,0]-0.5,ROI[1,0]-0.5,ROI[1,1]-0.5,ROI[1,1]-0.5,ROI[1,0]-0.5],'k')
					plt.colorbar()
					plt.pause(0.01)

					plt.figure()
					plt.imshow(np.flip(np.transpose(foilthickness,(1,0)),axis=1),'rainbow',origin='lower')
					plt.colorbar()
					plt.pause(0.01)
				...
				all_total_radiated_power.append(total_radiated_power)
			...
			# plt.figure()
			# plt.scatter(np.mean(grid_data_masked,axis=1)[:,0],np.mean(grid_data_masked,axis=1)[:,1],c=all_m[i_t],marker='s')
			# plt.pause(0.01)
			#
			# plt.figure()
			# plt.imshow(recompose_voxel_emissivity[i_t],'rainbow',origin='lower')
			# plt.pause(0.01)
			...
			# plt.semilogy()
			# plt.xlabel('time [s]')
			# plt.ylabel('Regularisation factor alpha [au]')
			# plt.grid()
			# plt.savefig(path_for_plots + '/Alpha_grid'+str(grid_resolution)+'cm.eps')
			# plt.close()

			inverted_dict[str(grid_resolution)][str(shrink_factor_x)][str(shrink_factor_t)]['recompose_voxel_emissivity'] = recompose_voxel_emissivity
			inverted_dict[str(grid_resolution)][str(shrink_factor_x)][str(shrink_factor_t)]['all_alpha'] = all_alpha
