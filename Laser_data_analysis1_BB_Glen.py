...
if False:	# manual collection of parameters
	...
	cases_to_include = ['laserG01','laserG02','laserG03']
	...
	all_laser_to_analyse_absorber_material = []
	for name in cases_to_include:
		...
		all_laser_to_analyse_absorber_material.extend([collection_of_records[name]['absorber_material']])
	path_to_save_figures_master = '/home/ffederic/work/irvb/laser/results_Glen/'

# Glen uses a variable framerate and int time, so I'll need to find by hand a kind of correct proportional coefficient assuming emissivity=1 and a standard emperature =25deg
...
def function_a(index):
	...
	absorber_material = all_laser_to_analyse_absorber_material[index]
	if absorber_material == 'Au':
		absorber_conductivity = Authermalconductivity
	elif absorber_material == 'Pt':
		absorber_conductivity = Ptthermalconductivity
	else:
		absorber_conductivity = Ptthermalconductivity
	print('absorber material is '+absorber_material)
	...
	for type_of_calibration in ['BB_source_w_window']:
		...
		if index==0:	# Glen tells me that this is the case. the metadata is wrong as the connection was via lan and too slow
			laser_framerate = 77.5
		if index==1:
			laser_framerate = 200
		if index==2:
			laser_framerate = 31
		print('measured framerate from timestamp=%.3gHz comparet to theoretical one of %.3gHz' %(1/(np.diff(time_partial).mean()),laser_framerate))
		...
		background_temperatures = 25
		background_counts = coleval.read_IR_file(path_reference_frames[0])['data_time_avg_counts']
		background_counts_std = coleval.read_IR_file(path_reference_frames[0])['data_time_avg_counts_std']
		relevant_background_files = [path_reference_frames[0]]
		reference_background_full_1,background_digitizer_ID = coleval.separate_data_with_digitizer(np.load(relevant_background_files[0]+'.npz'))

		# manual edit to make things faster
		if index==0:
			background_counts = background_counts[:,250:,100:]
			background_counts_std = background_counts_std[:,250:,100:]
			full_saved_file_dict_data = full_saved_file_dict['data'][:,250:,100:]
			reference_background_full_1 = np.array(reference_background_full_1)[:,:,250:,100:]
		elif index==1:
			background_counts = background_counts[:,:150]
			background_counts_std = background_counts_std[:,:150]
			full_saved_file_dict_data = full_saved_file_dict['data'][:,:150]
			reference_background_full_1 = np.array(reference_background_full_1)[:,:,:150]
		elif index==2:
			background_counts = background_counts[:,150:400,500:900]
			background_counts_std = background_counts_std[:,150:400,500:900]
			full_saved_file_dict_data = full_saved_file_dict['data'][:,150:400,500:900]
			reference_background_full_1 = np.array(reference_background_full_1)[:,:,150:400,500:900]


		# Calculating the background image
		reference_background = background_counts
		reference_background_std = background_counts_std
		reference_background_flat = np.nanmean(reference_background,axis=(1,2))
		reference_background_camera_temperature = background_temperatures

		max_ROI = [[0,np.shape(reference_background_full_1)[2]-1],[0,np.shape(reference_background_full_1)[3]-1]]
		foil_position_dict['foilcenter'] = ((np.flip(np.shape(reference_background[0]),axis=0)/2).astype(int))# + np.array([0,100])).tolist()
		foil_position_dict['foilhorizwpixel'] = np.shape(reference_background[0])[1]-2
		foil_position_dict['foilhorizw'] = 0.09
		foil_position_dict['foilvertw'] = 0.03	# 0.07

		flag_1 = [coleval.find_dead_pixels([data],treshold_for_bad_difference=100) for data in reference_background_full_1]
		bad_pixels_flag = flag_1
		...
		plt.plot(foilcenter[0],foilcenter[1],'k+',markersize=30)
		figure_index+=1
		plt.savefig(path_to_save_figures+laser_to_analyse[-6:] + path_to_save_figures2 + 'FIG'+str(figure_index)+'.eps')
		plt.close('all')
		...
		laser_counts_filtered = cp.deepcopy(full_saved_file_dict_data+full_saved_file_dict['data_median'])
		laser_digitizer = cp.deepcopy(full_saved_file_dict['digitizer_ID'])
		...
		# for emissivity in emissivity_range[7:]:
			...
			if type_of_calibration == 'BB_source_w_window':
				params_BB = np.ones((1,*np.shape(reference_background[0]),2))
				# params_BB[:,:,:,0] *= 2.5e-14	# adapted by hand do give ~25deg
				params_BB[:,:,:,0] *= 1.4e-14	# adapted by hand do give ~25deg
				errparams_BB = np.zeros((1,*np.shape(reference_background[0]),2,2))
			...
			# for reference_temperature in reference_temperature_range[:2]:
				...
				ref_temperature = np.mean(reference_background_temperature)
				ref_temperature_std = 0.1

				reference_background_temperature_crop = reference_background_temperature
				reference_background_temperature_std_crop = reference_background_temperature_std
				if len(laser_counts_filtered)>500:	# this is to avoid going out of memory
					if type_of_calibration == 'BB_source_w_window' or type_of_calibration == 'BB_source_w/o_window':
						...
						for i in np.arange(1,np.ceil(len(laser_counts_filtered)/500)).astype(int):
				...
				time_of_experiment_digitizer_ID = np.arange(len(laser_temperature_std_no_dead_pixels[0]))*(1/laser_framerate)/1e-6	# microsends
				time_partial = [(time_of_experiment_digitizer_ID-np.min(time_of_experiment_digitizer_ID))*1e-6]	# s
				...
				if True:	# spatially resolved foil properties from Japanese producer
					...
					conductivityscaled=np.multiply(absorber_conductivity,np.ones((foilvertwpixel-2,foilhorizwpixel-2)))
					...
					conductivityscaled=absorber_conductivity*np.ones((foilvertwpixel,foilhorizwpixel))
				...
				if False:	# I add this for the plots in the paper
					...
					plt.savefig(path_to_save_figures+laser_to_analyse[-6:] + path_to_save_figures2 + 'FIG_for_paper_1'+'.eps')
				else:
					pass
				...
				output1 = [coleval.calc_temp_to_power_BB_2(dTdt[i],dTdt_std[i],d2Tdxy[i],d2Tdxy_std[i],negd2Tdxy[i],negd2Tdxy_std[i],T4_T04[i],T4_T04_std[i],nan_ROI_mask,1,1,1/1,absorber_conductivity) for i in range(len(laser_digitizer_ID))]
				temp_BBrad = [[]]
				temp_diffusion = [[]]
				temp_timevariation = [[]]
				temp_powernoback = [[]]
				...
				if False:	# other plots for the paper. i think i want to show that the area where the diffusion components cancels out grows in time
					...
					for delta_time in [0,10,20,40,60,90,130,170,210]:
						...
						plt.plot(np.arange(len(temp))*dx*1000,np.array(temp)*2.5/1000000*absorber_conductivity*(dx**2),label='pulse %+.0fms' %((delta_time-1)/laser_framerate*1000))
					plt.legend(loc='best', fontsize='small')
					plt.grid()
					plt.xlim(left=-0.1,right=8)
					# plt.ylim(bottom=-0.1,top=2)
					...
					plt.axvline(x=20*dx*1000,color='y',linestyle='--')
					plt.title(preamble_4_prints+'Laser spot location in '+laser_to_analyse+'\n foil size '+str([foilhorizwpixel,foilvertwpixel])+'pixels, [%.3g,%.3g]mm\n laser located at [%.3g,%.3g]mm prelim radious %.3gmm\n' %(foilhorizwpixel*dx*1e3,foilvertwpixel*dx*1e3,horizontal_loc*dx*1e3,vertical_loc*dx*1e3,dr*dx*1e3))
					plt.savefig(path_to_save_figures+laser_to_analyse[-6:] + path_to_save_figures2 + 'FIG_for_paper_2'+'.eps')


				dr_total_power_minimum = 9
				dr_total_power = 20	# fixed at 50 so it's the same for all
				if index==2:
					dr_total_power_minimum = 15	# this is slightly larger thanthe size of the pinhole
					dr_total_power = 25	# this is slightly larger thanthe size of the pinhole



				if index==0:
					frames_for_one_pulse = 77.5/0.015
				if index==1:
					frames_for_one_pulse = 200/0.12
				if index==2:
					frames_for_one_pulse = 31/0.4
