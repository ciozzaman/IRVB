...
for cases_to_include in all_cases_to_include:
	...
	for max_laser_frequency_focused in threshold_freq_list:
		...
		# y = np.array([np.ones_like(all_laser_to_analyse_power_end),np.ones_like(all_laser_to_analyse_power_end),np.ones_like(all_laser_to_analyse_power_end)]).T.flatten()
		y = np.array([np.ones_like(all_laser_to_analyse_power_end),np.ones_like(all_laser_to_analyse_power_end),best_sharpness]).T.flatten()
		# weigth = np.ones_like(y)
		...
		fitted_coefficients = list(fitted_coefficients[:2]) + [power_reduction_window] + list(fitted_coefficients[2:])
		# print('found coefficients: thickness '+str(fitted_coefficients[0])+'m, emissivity '+str(fitted_coefficients[1]))
		print(fitted_coefficients)
		fitted_coefficients_2 = fit[0]
		fitted_coefficients_2 = fitted_coefficients_2[:2].tolist() + [power_reduction_window] + fitted_coefficients_2[2:].tolist()
		total_output = calculate_laser_power_given_parameters(1,*fit[0])
		all_laser_to_analyse_power_end *= fitted_coefficients_2[2]
		...
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused'],total_output[1::2][all_focus_status_end=='fully_defocused'],'o',color='r')
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='partially_defocused'],total_output[1::2][all_focus_status_end=='partially_defocused'],'v',color='r')
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='focused'],total_output[1::2][all_focus_status_end=='focused'],'+',color='r')
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused'],total_output[0::2][all_focus_status_end=='fully_defocused'],'o',color='b')
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='partially_defocused'],total_output[0::2][all_focus_status_end=='partially_defocused'],'v',color='b')
		ax1.plot(all_laser_to_analyse_power_end[all_focus_status_end=='focused'],total_output[0::2][all_focus_status_end=='focused'],'+',color='b')
		ax1.plot(np.sort(all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused']),np.sort(all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused']),'--',color='k')
		ax1.plot(np.sort(all_laser_to_analyse_power_end[all_focus_status_end!='fully_defocused']),np.sort(all_laser_to_analyse_power_end[all_focus_status_end!='fully_defocused']),'--',color='k')
		# ax1.plot(np.sort(all_laser_to_analyse_power_end)*fitted_coefficients_2[3],np.sort(all_laser_to_analyse_power_end),'--k')
		ax1.plot(np.sort(all_laser_to_analyse_power_end),np.zeros_like(all_laser_to_analyse_power_end),'--k')
		ax2.plot(all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused'],sharpness_indicator[all_focus_status_end=='fully_defocused'],'o',color='g')
		ax2.plot(all_laser_to_analyse_power_end[all_focus_status_end=='partially_defocused'],sharpness_indicator[all_focus_status_end=='partially_defocused'],'v',color='g')
		ax2.plot(all_laser_to_analyse_power_end[all_focus_status_end=='focused'],sharpness_indicator[all_focus_status_end=='focused'],'+',color='g')
		ax2.plot(np.sort(all_laser_to_analyse_power_end),np.ones_like(all_laser_to_analyse_power_end),'--g')
		...
		ax1.plot((all_laser_to_analyse_duty_end/all_laser_to_analyse_frequency_end)[all_focus_status_end=='fully_defocused'],all_laser_to_analyse_power_end[all_focus_status_end=='fully_defocused'],'ok')
		ax1.plot((all_laser_to_analyse_duty_end/all_laser_to_analyse_frequency_end)[all_focus_status_end=='partially_defocused'],all_laser_to_analyse_power_end[all_focus_status_end=='partially_defocused'],'vk')
		ax1.plot((all_laser_to_analyse_duty_end/all_laser_to_analyse_frequency_end)[all_focus_status_end=='focused'],all_laser_to_analyse_power_end[all_focus_status_end=='focused'],'+k')
	...
	ax1.plot(threshold_freq_list,np.ones_like(threshold_freq_list)*2.5e-6,'--b')
	ax1.plot(threshold_freq_list,np.array(coefficients_first_stage)[:,0],'xb')
	a1, = ax1.plot(threshold_freq_list,np.array(coefficients)[:,0],'b')
	ax2.plot(threshold_freq_list,np.ones_like(threshold_freq_list)*1,'--r')
	a2, = ax2.plot(threshold_freq_list,np.array(coefficients)[:,1],'r')
	a3, = ax3.plot(threshold_freq_list,np.array(coefficients)[:,3],'og',label='defocused/focused factor')
	a3, = ax3.plot(threshold_freq_list,all_R2,'*g',label='R2')
	a3, = ax3.plot(threshold_freq_list,np.array(coefficients)[:,2],'xg',label='window transmissivity')
	a3, = ax3.plot(threshold_freq_list,all_sharpness_first,'vg',label='average sharpness first pass')
	a3, = ax3.plot(threshold_freq_list,all_sharpness_second,'^g',label='average sharpness second pass')
	ax4.plot(threshold_freq_list,np.ones_like(threshold_freq_list)*Ptthermaldiffusivity,'--k')
	ax4.plot(threshold_freq_list,np.array(coefficients_first_stage)[:,1],'xk')
	a4, = ax4.plot(threshold_freq_list,np.array(coefficients)[:,4],'k')
	...
	ax3.set_ylabel('factors [au]', color=a3.get_color())  # we already handled the x-label with ax1
	ax4.set_ylabel('fitted diffusivity [m2/s]', color=a4.get_color())  # we already handled the x-label with ax1
	...
	ax3.legend(loc='center right', fontsize='x-small')
	...
	for index in np.arange(len(all_laser_to_analyse_power_end)):
		...
		if experimental_laser_duty!=0.5:
			totalpower_filtered_1_full_negative = generic_filter(powernoback,np.mean,size=[max(1,int(frames_for_one_pulse*(1-experimental_laser_duty)/15//2*2+1))])
