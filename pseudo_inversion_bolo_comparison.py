...
for spatial_averaging in spatial_averaging_all:
	...
	for radiator_location in radiator_location_all:
		...
		for time_averaging in time_averaging_tries:
			...
			if (flag_mask2_present and is_this_extra):
			...
			for index in range(num_voxels):
				...
				if ((voxel_centre.x-radiator_location[0])**2+(voxel_centre.y-radiator_location[1])**2)<((radiator_diameter/4)**2):
					power_on_voxels[index] = radiator_emissivity
			if (flag_mask2_present and is_this_extra):
				...
				for index in range(num_voxels):
					...
					voxel_centre = Point2D((p1.x + p2.x + p3.x + p4.x) / 4, (p1.y + p2.y + p3.y + p4.y) / 4)
					if ((voxel_centre.x - radiator_location[0]) ** 2 + (voxel_centre.y - radiator_location[1]) ** 2) < ((radiator_diameter/4)**2):
						power_on_voxels_no_mask[index] = radiator_emissivity
			else:
			...
			if flag_mask2_present:
				path_sensitivity = path_sensitivity + '/mask2'
			...
			if treshold_method_try_to_search:
				...
				if eigenvalue_cut_vertical:
					path_sensitivity = path_sensitivity + '/SVG_vertical_cut'
					path_for_plots = path_for_plots + '/SVG_vertical_cut'
					print('Cut of the eigenvalues in the vertical direction')
				else:
					path_sensitivity = path_sensitivity + '/SVG_horizontal_cut'
			...
			if residuals_on_power_on_voxels:
				path_sensitivity = path_sensitivity + '/residuals_on_power_on_voxels'
				...
				path_sensitivity = path_sensitivity + '/residuals_on_power_on_foil_pixels'
			...
			to_write = [['grid resolution in cm', str(grid_resolution)],
						...
						['fraction of the total theoretical power detected',str(total_estimated_radiated_power/radiator_power)]
						]
