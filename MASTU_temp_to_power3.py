...
for shrink_factor_t in [1,2,3]:
	for shrink_factor_x in [1,5,10]:
		...
		for i in range(len(laser_digitizer_ID)):
			...
			select_time = np.logical_and(time_partial_binned>0,time_partial_binned<1.5)
			...
			laser_temperature_minus_background_crop_binned = laser_temp_filtered[select_time]-reference_background_temperature_crop_binned
			laser_temperature_std_minus_background_crop_binned = (laser_temp_filtered_std[select_time]**2+reference_background_temperature_std_crop_binned**2)**0.5
			...
			for i in range(len(fueling_point_location_on_foil)):
				plt.plot(np.array(fueling_point_location_on_foil[i][:,0])*(np.shape(temp2)[1]-1)/foil_size[0],np.array(fueling_point_location_on_foil[i][:,1])*(np.shape(temp2)[0]-1)/foil_size[1],'+k',markersize=40,alpha=0.5)
				plt.plot(np.array(fueling_point_location_on_foil[i][:,0])*(np.shape(temp2)[1]-1)/foil_size[0],np.array(fueling_point_location_on_foil[i][:,1])*(np.shape(temp2)[0]-1)/foil_size[1],'ok',markersize=5,alpha=0.5)
			for i in range(len(structure_point_location_on_foil)):
				plt.plot(np.array(structure_point_location_on_foil[i][:,0])*(np.shape(temp2)[1]-1)/foil_size[0],np.array(structure_point_location_on_foil[i][:,1])*(np.shape(temp2)[0]-1)/foil_size[1],'--k',alpha=0.5)
		...
		saved_file_dict_short[binning_type]['powernoback'] = np.float32(powernoback)
		saved_file_dict_short[binning_type]['powernoback_std'] = np.float32(powernoback_std)
		...
		temp = np.sort(powernoback_full[:,:,:int(np.shape(powernoback_full)[2]*0.75)].flatten())
		ani = coleval.movie_from_data(np.array([np.flip(np.transpose(powernoback_full,(0,2,1)),axis=2)]), laser_framerate_binned,integration=laser_int_time/1000,time_offset=time_full_binned[1],extvmin=0,extvmax=np.mean(temp[-len(temp)//60:]),xlabel='horizontal coord [pixels]', ylabel='vertical coord [pixels]',barlabel='Power on foil [W/m2]', prelude='shot ' + laser_to_analyse[-9:-4] + '\n',overlay_structure=True,include_EFIT=True,pulse_ID=laser_to_analyse[-9:-4],overlay_x_point=True,overlay_mag_axis=True)
		...
		temp = np.sort(BBrad_full[:,:,:int(np.shape(BBrad_full)[2]*0.75)].flatten())
		ani = coleval.movie_from_data(np.array([np.flip(np.transpose(BBrad_full,(0,2,1)),axis=2)]), laser_framerate_binned,integration=laser_int_time/1000,time_offset=time_full_binned[1],extvmin=0,extvmax=np.mean(temp[-len(temp)//60:]),xlabel='horizontal coord [pixels]', ylabel='vertical coord [pixels]',barlabel='Power BB on foil [W/m2]', prelude='shot ' + laser_to_analyse[-9:-4] + '\n',overlay_structure=True,include_EFIT=True,pulse_ID=laser_to_analyse[-9:-4],overlay_x_point=True,overlay_mag_axis=True)
		...
		temp = np.sort(diffusion_full[:,:,:int(np.shape(diffusion_full)[2]*0.75)].flatten())
		ani = coleval.movie_from_data(np.array([np.flip(np.transpose(diffusion_full,(0,2,1)),axis=2)]), laser_framerate_binned,integration=laser_int_time/1000,time_offset=time_full_binned[1],extvmin=0,extvmax=np.mean(temp[-len(temp)//60:]),xlabel='horizontal coord [pixels]', ylabel='vertical coord [pixels]',barlabel='Power on foil [W/m2]', prelude='shot ' + laser_to_analyse[-9:-4] + '\n',overlay_structure=True,include_EFIT=True,pulse_ID=laser_to_analyse[-9:-4],overlay_x_point=True,overlay_mag_axis=True)
		...
		temp = np.sort(timevariation_full[:,:,:int(np.shape(timevariation_full)[2]*0.75)].flatten())
		ani = coleval.movie_from_data(np.array([np.flip(np.transpose(timevariation_full,(0,2,1)),axis=2)]), laser_framerate_binned,integration=laser_int_time/1000,time_offset=time_full_binned[1],extvmin=0,extvmax=np.mean(temp[-len(temp)//60:]),xlabel='horizontal coord [pixels]', ylabel='vertical coord [pixels]',barlabel='Power time deriv. diffusion on foil [W/m2]', prelude='shot ' + laser_to_analyse[-9:-4] + '\n',overlay_structure=True,include_EFIT=True,pulse_ID=laser_to_analyse[-9:-4],overlay_x_point=True,overlay_mag_axis=True)
