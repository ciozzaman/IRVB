...
try:
	...
	try:
		test = full_saved_file_dict['only_pulse_data']['laser_temperature_std_minus_background_full_minus_median']
		...
		for i in range(len(laser_digitizer_ID)):
			...
			if external_clock_marker:
				...
				if False:	# big failure, it fails to clean up the oscillation
					...
					filter_agent = np.mean(laser_counts_corrected[i][:,select_space],axis=-1) - generic_filter(np.mean(laser_counts_corrected[i][:,select_space],axis=-1),np.nanmean,size=[7])
					...
					filter_agent = np.mean(laser_counts_filtered[:,select_space],axis=-1) - generic_filter(np.mean(laser_counts_filtered[:,select_space],axis=-1),np.nanmean,size=[15])
