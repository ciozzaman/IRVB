...
class mclass:
    def __init__(self,  window):
        ...
        self.open_button = tk.Button(self.toolbar, text="Open File", command=self.open)
        self.shotlabel1 = tk.Label(self.toolbar, text = ' -or- shot: ')
        self.shot_box = tk.Entry(self.toolbar)
        self.shot_box.bind("<Return>", self.open_shot)
        ...
        self.shotlabel1.pack(side=tk.LEFT)
        self.shot_box.pack(side=tk.LEFT)
    ...
    def plot (self):
        ...
        if self.status[tindx] != 1:
            print("ERROR>> Timeslice {} is not converged".format(self.time[tindx]))
    ...
    def open_netcdf_uda(self, shot, file_prefix='epm'):

        import pyuda
        client = pyuda.Client()

        try:
            self.status = client.get('/'+file_prefix+'/equilibriumStatusInteger', shot).data
        except pyuda.UDAException:
            print("ERROR>> No data from {} for {}".format(file_prefix, shot))
            return

        ind_converged, = np.where(self.status == 1)
            
        if len(ind_converged) == 0:
            print("ERROR>> No converged timeslices")
            f.close()
            return

        self.shotnumber = shot

        try:
            self.psidat = np.transpose(client.get('/'+file_prefix+'/output/profiles2D/poloidalFlux', shot).data, (0, 2, 1))
            self.q0 = client.get('/'+file_prefix+'/output/globalParameters/q0', shot).data
            self.q95 = client.get('/'+file_prefix+'/output/globalParameters/q95', shot).data
            self.R = client.get('/'+file_prefix+'/output/profiles2D/r', shot).data
            self.Z = client.get('/'+file_prefix+'/output/profiles2D/z', shot).data
            self.bVac = client.get('/'+file_prefix+'/input/bVacRadiusProduct', shot).data
            self.r_axis = client.get('/'+file_prefix+'/output/globalParameters/magneticAxis/R', shot).data
            self.z_axis = client.get('/'+file_prefix+'/output/globalParameters/magneticAxis/Z', shot).data 
            self.psi_bnd = client.get('/'+file_prefix+'/output/globalParameters/psiBoundary', shot).data
            self.psi_axis = client.get('/'+file_prefix+'/output/globalParameters/psiAxis', shot).data
            self.cpasma = client.get('/'+file_prefix+'/output/globalParameters/plasmaCurrent', shot).data
            self.time = client.get('/'+file_prefix+'/time', shot).data
        except pyuda.UDAException:
            print("ERROR>> No data from {} for {}".format(file_prefix, shot))      
            return
        ...
        ind_first_converged = ind_converged[0]
        tkvar = tk.IntVar()
        tkvar.set(ind_first_converged)
        self.time_slider.config(textvariable=tkvar)
#        self.time_slider.set(ind_first_converged)
        self.plot()


    def open_netcdf(self, filename):                

        f = netCDF4.Dataset(filename)


        self.status = f['/epm/equilibriumStatusInteger'][:].data
...
#        self.time_slider.set(ind_first_converged)
    ...
    def open_shot(self, event):

        try:
            self.efit_filename = "/common/uda-scratch/lkogan/efitpp_eshed/epm0{}.nc".format(self.shot_box.get())
            self.open_netcdf(self.efit_filename)
        except FileNotFoundError:
            self.open_netcdf_uda(self.shot_box.get())
    ...
    def launch_fieldlines(self):
        ...
        R, Z, BR, BZ, Bphi = self.calc_bfield(self.psidat[tindx,:,:].T, self.R, self.Z, self.bVac[tindx])
