...
minimum_ON_period_list = 0.09	# seconds
...
# also laser25 100 has something wrong
...
all_cases_to_include = [['laser19','laser22','laser34','laser41','laser42','laser43','laser44','laser45','laser46','laser47','laser30','laser33']]	# all cases with correct fraq and no windowing
for cases_to_include in all_cases_to_include:
	...
	for index in range(len(all_laser_to_analyse)):
		...
		try:
			laser_dict = np.load(laser_to_analyse+'.npz')
			partial_BBrad = laser_dict['partial_BBrad_small']/sample_properties['emissivity']
			partial_BBrad_std = laser_dict['partial_BBrad_std_small']/sample_properties['emissivity']
			partial_diffusion = laser_dict['partial_diffusion_small']/sample_properties['thickness']
			partial_diffusion_std = laser_dict['partial_diffusion_std_small']/sample_properties['thickness']
			partial_timevariation = laser_dict['partial_timevariation_small']/((1/sample_properties['diffusivity'])*sample_properties['thickness'])
			partial_timevariation_std = laser_dict['partial_timevariation_std_small']/((1/sample_properties['diffusivity'])*sample_properties['thickness'])
	...
	def calculate_laser_power_given_parameters_1(trash,search_emissivity,search_thickness,search_diffusivity,search_defocused_to_focused_power):
		...
		for index in range(len(all_laser_to_analyse_power_end)):
			...
			if False:
				...
				sharpness_indicator = ((np.std(totalpower_filtered_1[totalpower_filtered_1>through+(peak+through)/2]) + np.std(totalpower_filtered_1[totalpower_filtered_1<through+(peak+through)/2]) + (all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power)))/(all_laser_to_analyse_power_end[index]*power_reduction_window*defocused_to_focused_power)
	...
	sigma = np.array([sigma,sigma,sigma*2]).T.flatten()
	bds = [[0.7,0.1*2.5e-6,0.1*Ptthermaldiffusivity,0.4],[1,10*2.5e-6,10*Ptthermaldiffusivity,1.2]]
	# guess=[0.98,2.5e-6,Ptthermaldiffusivity,1]
	guess=[0.98,2.0e-6,1e-5,1]
	fit = curve_fit(calculate_laser_power_given_parameters_1, x, y, sigma=sigma, p0=guess,bounds=bds,maxfev=int(1e6),verbose=2,diff_step=np.array(guess)/10,xtol=1e-10)
	guess = fit[0]
