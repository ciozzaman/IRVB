...
if False:	# manual collection of parameters
	...
	cases_to_include = ['laser22','laser35']
	all_case_ID = []
...
def function_a(index):
	...
	def power_vs_space_sampling_explorer():
		...
		for dr in (np.nanmean([dhorizontal_loc,dvertical_loc])*area_multiplier):
			...
			totalpower_filtered_1 = generic_filter(temp_powernoback[:int(len(totalpower)//frames_for_one_pulse*frames_for_one_pulse)],np.mean,size=[max(1,int(laser_framerate/experimental_laser_frequency*experimental_laser_duty/15//2*2+1))])
	...
	full_saved_file_dict['time_of_experiment'] = time_partial
